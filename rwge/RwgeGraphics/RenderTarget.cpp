...
RenderTarget::RenderTarget(const DisplayWindow* pWindow) :
	D3D9Device							(*pWindow),
	m_pWindow							(pWindow),
	...
	m_pCurrentIndexBuffer			(nullptr)
{
	...
	m_listVertexStreamBuffers.push_back(new VertexBuffer(*this, m_uVertexBufferSize));
...
void RenderTarget::ApplyVertexStreamAndIndexStream(VertexDeclaration* pVertexDeclarationInstance)
{
	if (m_pCurrentVertexDeclaration != pVertexDeclarationInstance)
	{
		...
		unsigned int uMissingBufferCount = uNewStreamCount - m_listVertexStreamBuffers.size();
		...
		list<VertexBuffer*>::iterator itVertexStreamBuffer = m_listVertexStreamBuffers.begin();
		for ( ; uStreamIndex < uNewStreamCount; ++uStreamIndex, ++itVertexStreamBuffer)
		{
			m_pDevice->SetStreamSource(
				uStreamIndex,													// Stream ID
				(*itVertexStreamBuffer)->GetD3DVertexStreamBuffer(),			// 绑定的StreamBuffer
...
void RenderTarget::DrawPrimitive(const RenderPrimitive& primitive)
{
	...
	list<VertexBuffer*>::iterator itVertexBuffer = m_listVertexStreamBuffers.begin();
