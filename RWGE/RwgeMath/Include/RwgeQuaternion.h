...
public:
	RQuaternion();
	RQuaternion(float fX, float fY, float fZ, float fW);
	RQuaternion(const D3DXVECTOR3& rotateAxis, const AngleRadian& radianAngle);
	RQuaternion(const D3DXVECTOR3& xAxis, const D3DXVECTOR3& yAxis, const D3DXVECTOR3& zAxis);
	RQuaternion(const D3DXMATRIX& matrix);
	~RQuaternion();

	RQuaternion& SetByAxisAngle(const D3DXVECTOR3& rotateAxis, const AngleRadian& radianAngle);
	void ToAxisAngle(D3DXVECTOR3& outRotateAxis, AngleRadian& outRadianAngle) const;

	RQuaternion& SetByAxes(const D3DXVECTOR3& xAxis, const D3DXVECTOR3& yAxis, const D3DXVECTOR3& zAxis);
	void ToAxes(D3DXVECTOR3& outXAxis, D3DXVECTOR3& outYAxis, D3DXVECTOR3& outZAxis) const;

	RQuaternion& SetByRotationMatrix(const D3DXMATRIX& matrix);
	void ToRotationMatrix(D3DXMATRIX& outMatrix) const;

	RQuaternion operator+ (const RQuaternion& quaternion) const;   
	RQuaternion operator- (const RQuaternion& quaternion) const;
	RQuaternion operator- () const;
	RQuaternion operator* (float fScalar) const;
	RQuaternion operator* (const RQuaternion& quaternion) const;
	RQuaternion Inverse() const;		// 返回四元数的逆
	float Norm() const;				// 求四元数的范数，即四元数模的平方
	float Modulus() const;			// 求四元数的模
	RQuaternion& Normalise();		// 单位化四元数，并返回自身

	D3DXVECTOR3&& RotateVector(const D3DXVECTOR3& vector) const;

	__forceinline static RQuaternion* AxisAngleToQuaternion(RQuaternion* pOutQuaternion, const D3DXVECTOR3* pInAxis, const AngleRadian& inAngle);
	__forceinline static void QuaternionToAxisAngle(const RQuaternion* pInQuaternion, D3DXVECTOR3* pOutAxis, AngleRadian& outRadianAngle);

	__forceinline static RQuaternion* RotationMatrixToQuaternion(RQuaternion* pOutQuaternion, const D3DXMATRIX* pInMatrix);
	__forceinline static D3DXMATRIX* QuaternionToRotationMatrix(const RQuaternion* pInQuaternion, D3DXMATRIX* pOutMatrix);

	__forceinline static RQuaternion* GetRotationBetween(
...
__forceinline RQuaternion* RQuaternion::AxisAngleToQuaternion(RQuaternion* pOutQuaternion, const D3DXVECTOR3* pInAxis, const AngleRadian& inAngle)
{
...
__forceinline void RQuaternion::QuaternionToAxisAngle(const RQuaternion* pInQuaternion, D3DXVECTOR3* pOutAxis, AngleRadian& outRadianAngle)
{
...
__forceinline RQuaternion* RQuaternion::RotationMatrixToQuaternion(RQuaternion* pOutQuaternion, const D3DXMATRIX* pInMatrix)
{
...
__forceinline D3DXMATRIX* RQuaternion::QuaternionToRotationMatrix(const RQuaternion* pInQuaternion, D3DXMATRIX* pOutMatrix)
{
...
__forceinline RQuaternion* RQuaternion::GetRotationBetween(RQuaternion* pRotation, const D3DXVECTOR3& v1, const D3DXVECTOR3& v2, const D3DXVECTOR3& rotationAxis /* = RwgeMath::Vector3Zero */)
{
