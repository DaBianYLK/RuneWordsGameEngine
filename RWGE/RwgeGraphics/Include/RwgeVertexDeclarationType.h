...
类似于shader，D3D顶点声明与Device是绑定的，所以需要由RenderTarget进行统一管理，渲染图元中维护一个指向VertexDeclarationType的指针
...
public:
	unsigned short GetStreamCount() const;
	unsigned short GetElementCount() const;
	unsigned short GetStreamVertexSize(unsigned short uStreamID = 0) const;
	unsigned short GetTotalVertexSize() const;

	VertexDeclaration* GetVertexDeclarationInstance(RenderTarget* pRenderTarget);
...
private:
	...
	mutable bool m_bVertexDataOutOfDate;
	mutable unsigned short m_uElementCount;						// 顶点声明中元素的个数
	mutable std::vector<unsigned short> m_vecStreamVertexSize;	// 每个流中的顶点大小
	mutable unsigned short	m_uTotalVertexSize;					// 一个顶点的大小（所有流中顶点大小的和）
};